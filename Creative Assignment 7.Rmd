...
#why does it insist on putting the geometry in the pivoted column names??


bos_evict <- st_as_sf(inner_join(evictions, acs18_data, by = "GEOID"),
                      crs= lat_lon_CRS)
...
bos_evict$label <- 
  paste(bos_evict$NAME, "<br>", 
        bos_evict$filings_2020, " evictions filed in 2020") %>% 
...
polymap <- leaflet(tract_points) %>%
  ...
  addLegend(pal = pal, 
            values = ~filings_2020,
            bins = 3,
            opacity = 0.7, title = "Evictions Filings in 2020",
            position = "topright")
