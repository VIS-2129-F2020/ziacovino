...
#Points A: Accidents 
accidents <- read.csv("Vision_Zero_Crash_Records.csv")
...
#Ah geez the vision zero data doesn't work...:Error: stat_sf requires the following missing aesthetics: geometry

ggplot(zipcodes) +
  geom_sf(fill = NA, color = "black", size = 2)+ 
  geom_sf(data = parks, fill = "green")+ 
  geom_sf(data = ramps, size = 0.01) +
  #geom_sf(data= accidents, fill = "pink", color = "black") +
  theme_map() +
  annotation_scale()
...
#Does this count as the number of ramps in zipcodes? 
zipcodes <- zipcodes %>%
...
The number and proportion of [zip code] polygons that overlap with [open space] polygons.
```{r Calculations 4}
zipcodes <- zipcodes %>%
...
accidents <- accidents %>%
  ...
  mutate(tree_dist = as.numeric(ramps_dist))
